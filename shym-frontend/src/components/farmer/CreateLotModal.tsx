...
export default function CreateLotModal({ isOpen, onClose }: CreateLotModalProps) {
  ...
  const user = useQuery(api.users.getById, farmerId ? { id: farmerId } : "skip");
  ...
  React.useEffect(() => {
    if (isOpen && user?.address) {
      ...
      // reset logic here is partially handled on submit, but we can set address when opened
    }
  }, [isOpen, user?.address]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.message || 'Ошибка создания лота');
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 animate-in fade-in zoom-in-95 duration-200">
        <h2 className="text-xl font-bold text-[#2D2D2D] mb-6">Создание нового лота</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-4 border-t border-[#E8E4DE]">
            ...
            <Button type="submit" disabled={isSubmitting} className="flex-1">
              {isSubmitting ? 'Сохранение...' : 'Создать лот'}
            </Button>
          </div>
        </form>
      </div>
